...
def _decorate_with_tertiary(X, *args):
    ...
    X['pred_tertiary_class'] = args[1]
    X['pred_tertiary_score'] = args[2]
...
def _enforce_tertiary_hierarchy(row):
    ...
    secondary, s_score, tertiary, t_score = row['final_secondary'], row['pred_secondary_score'], \
        row['pred_tertiary_class'], row['pred_tertiary_score']
...
def main():
    ...
    X = df[['subject', 'text', 'event_type', 'final_primary', 'final_secondary',
            'pred_primary_score', 'pred_secondary_score']]
