...
def convert_df_to_sql(results, df):
    ...
    df_for_upload['predicted_primary_class'] = results[0]
    df_for_upload['predicted_primary_score'] = results[1]
...
def main():
    ...
    for _date in dates:
        ...
        query_events.create_staging_and_insert_primary(table_name, for_database.values.tolist())
    ...
    logger.info('Completed primary classifications of events from %s to %s', start_date, end_date)
