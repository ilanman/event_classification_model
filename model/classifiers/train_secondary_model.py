...
def get_secondary_testing_features(X, feature_pipeline):
    ...
    predicted_primary_matrix = feature_pipeline['predicted_primary'].transform(X.pred_primary_class)
...
def _decorate_with_secondary(X, *args):
    ...
    X['true_secondary_class'] = args[0]
    X['pred_secondary_class'] = args[1]
    X['pred_secondary_score'] = args[2]
    X['true_tertiary_class'] = args[3]
...
def _enforce_secondary_hierarchy(row):
    ...
    primary, secondary, p_score, s_score = row['pred_primary_class'], row['pred_secondary_class'],\
     row['pred_primary_score'], row['pred_secondary_score']
